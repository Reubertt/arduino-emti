/*
  ...
  3.  **CONTROLE DOS MOTORES (Ponte H para Arduino):**
      - **Motor Direito:**
          ...
          - L298N ENA  -> Arduino Digital 6 (MOTOR_DIREITO_ENA)
      - **Motor Esquerdo:**
          - L298N IN3  -> Arduino Digital 4 (MOTOR_ESQUERDO_IN3)
          - L298N IN4  -> Arduino Digital 5 (MOTOR_ESQUERDO_IN4)
          - L298N ENB  -> Arduino Digital 11 (MOTOR_ESQUERDO_ENB)
...
const int MOTOR_DIREITO_ENA =
    6; // Pino de Habilitação (Enable) do motor direito.
...
const int MOTOR_ESQUERDO_ENB =
    11; // Pino de Habilitação (Enable) do motor esquerdo.
...
void setup() {
  ...
  pinMode(MOTOR_DIREITO_ENA, OUTPUT);
...
// Estas funções controlam o movimento do robô, definindo a direção
// dos motores.
...
void girarNoEixo() {
  girarDireita(); // Reutiliza a função girarDireita para girar no eixo.
}
...
bool checkRingBoundary() {
  ...
  if (frontBoundaryDetected) {
    ...
    delay(100);
    girarDireita(); // Gira para a direita para se afastar da borda.
  ...
  // --- Lógica para Borda Traseira Detectada ---
    ...
    pararMotores(); // Para imediatamente.
    delay(100);
    avancar(); // Empurra para frente com velocidade máxima.
    delay(700); // Empurra por um tempo.
    pararMotores(); // Para.
    delay(100);
    girarDireita(); // Gira para a direita para reorientar, caso ainda esteja perto da borda.
    delay(400);
    pararMotores(); // Para após o giro.
    return true; // Indica que a borda foi detectada e tratada.
  }
